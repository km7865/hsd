...
function Agree(props) {
    ...
    function signUp() {
        console.log(agree1Ref.current.checked);
        if (!agree1Ref.current.checked) {
            return;
        }

        if (!agree2Ref.current.checked) {
            return;
        }

        window.location.href = "/signUp/" + agree3Ref.current.checked;
    }

    const agree1Ref = useRef(null);
    const agree2Ref = useRef(null);
    const agree3Ref = useRef(null);
    ...
    return (<>
        <div align="center">
            <div>
                <p><h3>이용약관</h3></p>
                <textarea readOnly rows="10" cols="60">{agree1}</textarea><br />
                <input type="checkbox" className="check" ref={agree1Ref}/><span>동의합니다 (필수)</span>
                <br />
            </div>
            <div>
                <p><h4>개인정보 수집 및 활용동의</h4></p>
                <textarea readOnly rows="10" cols="60">{agree2}</textarea><br />
                <input type="checkbox" ref={agree2Ref}/><span>동의합니다 (필수)</span>
                <br />
            </div>
            <div>
                <p><h4>마케팅 활용목적 동의</h4></p>
                <textarea readOnly rows="10" cols="60">{agree3}</textarea><br />
                <input type="checkbox" className="check" ref={agree3Ref}/><span>동의합니다 (선택)</span>
                <br />
            </div>
            
            <div className="all_check">
                <input type="checkbox" ref={agreeAllRef}/><span>위 약관에 모두 동의합니다</span>
                <button className="nextBtn" onClick={() => {signUp()}}>다음</button>
            </div>
